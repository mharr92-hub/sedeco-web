...
export function CaseCard({ caseItem, ctaHref = "/#contacto" }: CaseCardProps) {
  ...
  return (
    <article className="flex flex-col rounded-lg border border-ink-200 bg-white p-6 transition-colors hover:border-ink-300">
      ...
      {scope ? (
        <div className="mt-6">
          <p className="font-mono text-xs uppercase tracking-[0.2em] text-ink-400 mb-2">
            Alcance
          </p>
      ...
      {signedBy ? (
        <p className="mt-6 border-t border-ink-200 pt-5 text-xs text-ink-500">
          Carta de respaldo · {signedBy.name}
          <span className="block text-ink-400">{signedBy.role}</span>
        </p>
      ) : null}

      <div className="mt-6 pt-5 border-t border-ink-200 flex-1 flex items-end">
        <a
          href={ctaHref}
